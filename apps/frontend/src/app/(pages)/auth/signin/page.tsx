...
export default function SignIn() {
    ...
    return (
        <div className="relative">
            <div className="flex">
                <div className="w-[50%] h-screen flex justify-center items-center" style={{ backgroundImage: `url("https://img.freepik.com/premium-vector/hand-drawn-ink-chat-bubble-set-pattern_1027691-779.jpg?w=740")`, backgroundPosition: "center" }}>
                    <div className="w-[400px] h-[350px] bg-neutral-800 rounded-xl flex flex-col">
                        <div
                        ...
                        <div>
                            <h1 className="text-center text-slate-200">Don't have an account? <button onClick={() => {router.push("/auth/signup")}} className="text-blue-400 hover:underline hover:cursor-pointer">Sign Up</button></h1>
                        </div>
                        <div className="flex flex-col p-6 space-y-4 justify-center items-center w-full">
                            <Button width="100px" height="40px" text="Login" backgroundColor="#5e84f1" color="white" textSize="20px" onClick={login} />
                        </div>
                    </div>
                </div>
            ...
            {isModalOpen && (
                <Modal textSize="24px" color="white" backgroundColor="#1f1f1f" isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Error">
                    <div className="text-red-300 text-2xl">Invalid credentials</div>
                </Modal>
            )}
        </div>
    )
